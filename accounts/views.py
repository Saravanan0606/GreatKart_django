        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # Get cart items from the user to access their product variation
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            # send mail to the user for resetting his/her password
    ...
    if request.method=='POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                user.save()
                # auth.logout(request)
                messages.info(request,"Password changed successfully ! You are now logged out. Please login again with your new password.")
            ...
            return redirect('change_passwd')
    return render(request, 'accounts/change_passwd.html')
